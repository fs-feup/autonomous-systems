...
public:
  LimoWrapper()
    ...
    lidar_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        ...
        std::bind(&LimoWrapper::lidar_callback, this, std::placeholders::_1), lidar_opt);
    ...
    match_points_pub =
        this->create_publisher<visualization_msgs::msg::MarkerArray>("/fast_limo/match_points", 1);
...
private:
  ...
  void correctLidarZ90Rotation(pcl::PointCloud<PointT>& cloud) {
  ...
  void lidar_callback(const sensor_msgs::msg::PointCloud2& msg) {
    ...
    // Correct pointcloud orientation if needed
    correctLidarZ90Rotation(*pc_);
    ...
    visualization_msgs::msg::MarkerArray match_markers =
        this->getMatchesMarker(loc.get_matches(), this->world_frame);
  ...
  void imu_callback(const sensor_msgs::msg::Imu& msg) {
    ...
    this->fromLimoToROS(loc.getWorldState(), loc.getPoseCovariance(), loc.getTwistCovariance(),
                        state_msg);
    this->fromLimoToROS(loc.getBodyState(), loc.getPoseCovariance(), loc.getTwistCovariance(),
                        body_msg);
    ...
    // TF broadcasting
    if (this->publish_tf) this->broadcastTF(loc.getWorldState(), world_frame, body_frame, true);
  }
  ...
  void loadConfig(fast_limo::Config* config) {
    ...
    config->extrinsics.imu2baselink_t =
        std::vector<float>(imu2baselink_t.begin(), imu2baselink_t.end());
    ...
    config->extrinsics.imu2baselink_R =
        std::vector<float>(imu2baselink_R.begin(), imu2baselink_R.end());
    ...
    config->extrinsics.lidar2baselink_t =
        std::vector<float>(lidar2baselink_t.begin(), lidar2baselink_t.end());
    ...
    config->extrinsics.lidar2baselink_R =
        std::vector<float>(lidar2baselink_R.begin(), lidar2baselink_R.end());
    ...
    config->filters.fov_angle = static_cast<float>(
        filt_fov_val_p.as_double() * M_PI / 360.0);  // half of FoV (bc. is divided by the x-axis)
  ...
  void fromROStoLimo(const sensor_msgs::msg::Imu& in, fast_limo::IMUmeas& out) {
    ...
    Eigen::Quaterniond qd(in.orientation.w, in.orientation.x, in.orientation.y, in.orientation.z);
    out.q = qd.cast<float>();
  }
  ...
  void broadcastTF(const fast_limo::State& in, std::string parent_name, std::string child_name,
                   bool now) {
  ...
  bool checkPointcloudStructure(const sensor_msgs::msg::PointCloud2& msg,
                                fast_limo::SensorType sensor) {
    ...
    if (sensor == fast_limo::SensorType::OUSTER) {
      ...
      RCLCPP_ERROR_STREAM(
          this->get_logger(),
          "\n-------------------------------------------------------------------\n"
              << "FAST_LIMO::FATAL ERROR: the received pointcloud MUST have a timestamp field "
                 "available!\n"
      ...
      RCLCPP_ERROR_STREAM(
          ...
          "\n-------------------------------------------------------------------\n"
              << "FAST_LIMO::FATAL ERROR: the received pointcloud MUST have a timestamp field "
                 "available!\n"
              << "          Remember that for VELODYNE alike pointclouds, the expected fields "
                 "are:\n"
              << "                  x: FLOAT32 (x coordinate in meters)\n"
              << "                  y: FLOAT32 (y coordinate in meters)\n"
              << "                  z: FLOAT32 (z coordinate in meters)\n"
              ...
              << "-------------------------------------------------------------------\n");
               ...
               (sensor == fast_limo::SensorType::LIVOX)) {
      for (size_t i = 0; i < msg.fields.size(); i++) {
      ...
      RCLCPP_ERROR_STREAM(
          ...
          "\n-------------------------------------------------------------------\n"
              << "FAST_LIMO::FATAL ERROR: the received pointcloud MUST have a timestamp field "
                 "available!\n"
              << "          Remember that for HESAI/LIVOX alike pointclouds, the expected fields "
                 "are:\n"
              << "                  x: FLOAT32 (x coordinate in meters)\n"
              << "                  y: FLOAT32 (y coordinate in meters)\n"
              << "                  z: FLOAT32 (z coordinate in meters)\n"
              << "                  timestamp: FLOAT64 (time since beginning of scan in "
                 "seconds/nanoseconds if HESAI/LIVOX)\n"
              << "-------------------------------------------------------------------\n");
