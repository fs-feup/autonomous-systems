...
cd ./ext/gtsam && mkdir -p build && cd build && cmake -DGTSAM_USE_SYSTEM_EIGEN=ON -DCMAKE_PREFIX_PATH="/usr/share/eigen3" -DCMAKE_BUILD_TYPE=Release -DGTSAM_POSE3_EXPMAP=ON -DCMAKE_CXX_FLAGS="-O3 -DNDEBUG" .. && sudo make check -j4 && sudo make install -j4 && cd ../../..
