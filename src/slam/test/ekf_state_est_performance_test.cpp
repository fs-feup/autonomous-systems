...
public:
  ...
  std::chrono::microseconds prediction_step_duration; /**< Duration of prediction step execution
                                                       */
  std::chrono::microseconds correction_step_duration; /**< Duration of correction step
execution */
  std::string workload;                               /**< Description of the
workload */
   ...
   */
  void print_to_file() {
    bool file_exists = std::filesystem::exists("../../performance/exec_time/slam/" + file_name);
    std::ofstream file("../../performance/exec_time/slam/" + file_name,
  ...
  void fill_x(int size) {
    ekf_test = std::make_shared<ExtendedKalmanFilter>(observation_model_, data_association_model_);
    ekf_test->add_motion_model("wheel_speed_sensor", motion_model_);
    std::ifstream file("../../src/slam/test/data/map_test.txt");
    ...
    ekf_test->_x_vector_ = Eigen::VectorXf::Zero(size * 2 + 6);
    ekf_test->_p_matrix_ = Eigen::SparseMatrix<float>(size * 2 + 6, size * 2 + 6);
    ...
    ekf_test->_p_matrix_.coeffRef(3, 3) = static_cast<float>(1.1);
    ekf_test->_p_matrix_.coeffRef(4, 4) = static_cast<float>(1.1);
    ekf_test->_p_matrix_.coeffRef(5, 5) = static_cast<float>(1.1);

    while ((file >> value1 >> value2) && i < (size * 2 + 6)) {
  ...
  void SetUp() override { /**< Set up test environment */
    file_name = "slam_" + get_current_date_time_as_string() + ".csv";
  ...
  void run_execution(const std::vector<common_lib::structures::Cone>& cone_map) {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      start_time = std::chrono::high_resolution_clock::now();
      ekf_for_predict.prediction_step(*motion_update_test, "wheel_speed_sensor");
  ...
  std::vector<common_lib::structures::Cone> create_cone_map(int n_cones) {
    ...
    std::ifstream file("../../src/slam/test/data/perception_output.csv");
    std::string line;
