 ...
 * pose
 */
class LinearFusionPoseUpdater : public PoseUpdater, public SecondPoseInputTrait {
...
public:
  explicit LinearFusionPoseUpdater(const SLAMParameters& params);
  ...
  std::shared_ptr<PoseUpdater> clone() const override {
    return std::make_shared<LinearFusionPoseUpdater>(*this);
  }
  ...
  bool second_pose_difference_ready() const { return !_waiting_to_calculate_average_; }
};
