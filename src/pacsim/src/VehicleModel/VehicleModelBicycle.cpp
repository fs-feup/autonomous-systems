...
{
    return powertrainModel.nominalVoltageTS;
}
...
double VehicleModelBicycle::PowertrainModel::calculateCurrent(const Wheels& torques, const Wheels& wheelspeeds, double voltage) const
{
...
void VehicleModelBicycle::calculateSlipAngles(double& kappaFront, double& kappaRear) const
{
    ...
    bool stillstand = (velocity.norm() < VELOCITY_THRESHOLD) && (std::abs(angularVelocity.z()) < ANGULAR_VELOCITY_THRESHOLD);
...
Eigen::Vector3d VehicleModelBicycle::getDynamicStates(double dt)
{
    ...
    double Fx_RL = (powertrainModel.gearRatio * torques.RL / powertrainModel.wheelRadius) * powertrainEfficiency;
    double Fx_RR = (powertrainModel.gearRatio * torques.RR / powertrainModel.wheelRadius) * powertrainEfficiency;
...
void VehicleModelBicycle::forwardIntegrate(double dt)
{
    ...
    wheelOrientations.FL = std::fmod(
        wheelOrientations.FL + wheelspeeds.FL * dt * wheelRotationFactor, TWO_PI);
    wheelOrientations.FR = std::fmod(
        wheelOrientations.FR + wheelspeeds.FR * dt * wheelRotationFactor, TWO_PI);
    wheelOrientations.RL = std::fmod(
        wheelOrientations.RL + wheelspeeds.RL * dt * wheelRotationFactor, TWO_PI);
    wheelOrientations.RR = std::fmod(
        wheelOrientations.RR + wheelspeeds.RR * dt * wheelRotationFactor, TWO_PI);
}
