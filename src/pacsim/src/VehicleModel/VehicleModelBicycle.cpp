...
void VehicleModelBicycle::setPowerGroundSetpoint(double in) { powerGroundSetpoint = std::clamp(in, 0.0, 1.0); }

void VehicleModelBicycle::setPosition(Eigen::Vector3d newPosition) { position = newPosition; }

void VehicleModelBicycle::setOrientation(Eigen::Vector3d newOrientation) { orientation = newOrientation; }
...
// tested
void VehicleModelBicycle::AerodynamicsModel::calculateForces(double velocityX, double& downforce, double& drag) const
{
...
double VehicleModelBicycle::TireModel::calculateLateralForce(double slipAngle, double normalForce) const
{
    if (slipAngle == 0.0)
    {
    ...
    double Kya = PKY1 * Fz_0_prime * (1 + PPY1 * dpi) * (1 - PKY3 * abs(y_input))
        * sin(PKY4 * atan((normalForce / Fz_0_prime) / ((PKY2 + PKY5 * y_input * y_input) * (1 + PPY2 * dpi)))) * zeta_3
        * LKY;
    ...
    double SHy = (PHY1 + PHY2 * dfz) + ((Kyg0 * y_input - SVyg) / (Kya + eps_k * sign(Kya))) * zeta_0 + zeta_4 - 1.0;
    double alpha_y = slipAngle + SHy;
    double Ey = (PEY1 + PEY2 * dfz) * (1 + PEY5 * y_input * y_input - (PEY3 + PEY4 * y_input) * sign(alpha_y)) * LEY;
    double Cy = PCY1 * LCY;
    double mu_y
        = (PDY1 + PDY2 * dfz) * (1.0 + PPY3 * dpi + PPY4 * dpi * dpi) * (1.0 - PDY3 * y_input * y_input) * LMUY_star;
    double Dy = mu_y * normalForce * zeta_2;
    double By = Kya / (Cy * Dy + eps_y * sign(Dy));
    double DVyk = mu_y * normalForce * (RVY1 + RVY2 * dfz + RVY3 * y_input) * cos(atan(RVY4 * slipAngle)) * zeta_2;
...
// tested
void VehicleModelBicycle::PowertrainModel::calculateWheelTorques(double throttleInput, Wheels& torques) const
{
...
// tested
double VehicleModelBicycle::PowertrainModel::calculateCurrent(
    const Wheels& torques, const Wheels& wheelspeeds, double voltage) const
{
...
// tested
void VehicleModelBicycle::SteeringModel::calculateSteeringAngles(double steeringInput, Wheels& steeringAngles) const
{
...
double VehicleModelBicycle::SteeringModel::calculateSteeringWheelAngle(const Wheels& steeringAngles) const
{
    ...
    return (steeringAngles.FL > 0) ? steeringAngles.FL / innerSteeringRatio : steeringAngles.FL / outerSteeringRatio;
}
...
/*
Currently this function depends on the longitudinal forces applied on the wheels and therefore can only be called after
calling calculateLongitudinalForces. This can be changed but it would imply calling the calculation of longitudinal
forces twice.

Also note that this only accounts for longitudinal weight transfer, because we are using a bycicle model , lateral
weight transfer cannot be applied as we only have one wheel in each axle.

To apply lateral weight transfer it would be necessary to account for each of the 4 wheels seperately.
*/
void VehicleModelBicycle::calculateWeightTransfer(
    double& Fz_Front, double& Fz_Rear, double& Fx_FL, double& Fx_FR, double& Fx_RL, double& Fx_RR) const
{
...
void VehicleModelBicycle::calculateSlipAngles(double& kappaFront, double& kappaRear) const
{
     ...
     */
    bool stillstand
        = (velocity.norm() < VELOCITY_THRESHOLD) && (std::abs(angularVelocity.z()) < ANGULAR_VELOCITY_THRESHOLD);
...
void VehicleModelBicycle::calculateWheelGeometry(double& steeringFront, Eigen::Vector3d& rFL, Eigen::Vector3d& rFR,
    Eigen::Vector3d& rRL, Eigen::Vector3d& rRR, Eigen::Vector3d& rFront, Eigen::Vector3d& rRear, Eigen::Vector3d& vFL,
    Eigen::Vector3d& vFR, Eigen::Vector3d& vRL, Eigen::Vector3d& vRR) const
{
    // Calculate average front steering angle
...
void VehicleModelBicycle::calculateLongitudinalForces(double& Fx_FL, double& Fx_FR, double& Fx_RL, double& Fx_RR) const
{
    ...
    Fx_RL = (powertrainModel.gearRatio * torques.RL / powertrainModel.wheelRadius) * powertrainEfficiency;
    Fx_RR = (powertrainModel.gearRatio * torques.RR / powertrainModel.wheelRadius) * powertrainEfficiency;
...
// tested
Eigen::Vector3d VehicleModelBicycle::calculateAccelerations(double steeringFront, double Fx_FL, double Fx_FR,
    double Fx_RL, double Fx_RR, double Fy_Front, double Fy_Rear, double drag, const Eigen::Vector3d& friction) const
{
    // Calculate longitudinal acceleration from tire forces
    double axTires = (std::cos(steeringAngles.FL) * Fx_FL + std::cos(steeringAngles.FR) * Fx_FR + Fx_RL + Fx_RR
                         - std::sin(steeringFront) * Fy_Front)
        / m;
...
void VehicleModelBicycle::updateWheelSpeeds(
    const Eigen::Vector3d& vFL, const Eigen::Vector3d& vFR, const Eigen::Vector3d& vRL, const Eigen::Vector3d& vRR)
{
...
Eigen::Vector3d VehicleModelBicycle::getDynamicStates(double dt)
{
    ...
    // Calculate weight transfer based on longitudinal forces -> this works because fz front and rear were calculate
    // above
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("a"),
        "Vx: %f, Vy: %f, W: %f, KappaF: %f, KappaR: %f, Fy_Front: %f, Fy_Rear: %f, drag: %f, friction: %f",
        velocity.x(), velocity.y(), angularVelocity.z(), kappaFront, kappaRear, Fy_Front, Fy_Rear, drag, friction.x());

    // Calculate accelerations
    return calculateAccelerations(steeringFront, Fx_FL, Fx_FR, Fx_RL, Fx_RR, Fy_Front, Fy_Rear, drag, friction);
}
...
void VehicleModelBicycle::forwardIntegrate(double dt)
{
    ...
    wheelOrientations.FL = std::fmod(wheelOrientations.FL + wheelspeeds.FL * dt * wheelRotationFactor, TWO_PI);
    wheelOrientations.FR = std::fmod(wheelOrientations.FR + wheelspeeds.FR * dt * wheelRotationFactor, TWO_PI);
    wheelOrientations.RL = std::fmod(wheelOrientations.RL + wheelspeeds.RL * dt * wheelRotationFactor, TWO_PI);
    wheelOrientations.RR = std::fmod(wheelOrientations.RR + wheelspeeds.RR * dt * wheelRotationFactor, TWO_PI);
}
