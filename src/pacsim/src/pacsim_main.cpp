...
public:
    ...
    int run() {
        ...
        while (rclcpp::ok() && !finish) {
            ...
            finish = competition_logic_->performAllChecks(
                track_landmarks, sim_time_, position, euler_angles);
            
            // Handle transforms and sensor updates
            publishTransforms(position, euler_angles, br);
            processDeadTimes();
            updateSensors(position, euler_angles, angular_acceleration, 
                          track_landmarks, track_as_lm_list, grip_map);
...
private:
    ...
    std::vector<std::string> joint_names_ = {
        "FL_steer", "FL_rotate", "FR_steer", "FR_rotate", 
    ...
    std::map<std::shared_ptr<ImuSensor>, rclcpp::Publisher<sensor_msgs::msg::Imu>::SharedPtr> imu_publisher_map_;
    std::map<std::shared_ptr<GnssSensor>, rclcpp::Publisher<pacsim::msg::GNSS>::SharedPtr> gnss_publisher_map_;
    ...
    bool fillMainConfig() {
        try {
            ...
            config["timeouts"].getElement<double>(&main_config_.timeout_trackdrive_first, "trackdrive_first");
            config["timeouts"].getElement<double>(&main_config_.timeout_trackdrive_total, "trackdrive_total");
    ...
    void setupPublishers() {
        ...
        velocity_pub_ = node_->create_publisher<geometry_msgs::msg::TwistWithCovarianceStamped>("/pacsim/velocity", 3);
        map_viz_pub_ = node_->create_publisher<visualization_msgs::msg::MarkerArray>("/pacsim/map", 1);
        
        // Setup sensor publishers
        steering_front_pub_ = node_->create_publisher<pacsim::msg::StampedScalar>("/pacsim/steeringFront", 1);
        steering_rear_pub_ = node_->create_publisher<pacsim::msg::StampedScalar>("/pacsim/steeringRear", 1);
        voltage_sensor_ts_pub_ = node_->create_publisher<pacsim::msg::StampedScalar>("/pacsim/ts/voltage", 1);
        current_sensor_ts_pub_ = node_->create_publisher<pacsim::msg::StampedScalar>("/pacsim/ts/current", 1);
        ...
        joint_state_publisher_ = node_->create_publisher<sensor_msgs::msg::JointState>("/joint_states", 3);
        
        for (auto& sensor : perception_sensors_) {
            auto detections_markers_wrapper = std::make_shared<LandmarksMarkerWrapper>(0.8, "pacsim/" + sensor->getName());
    ...
    void setupServices() {
        node_->create_service<std_srvs::srv::Empty>(
            ...
            std::bind(&PacSimNode::handleFinishSignal, this, std::placeholders::_1, std::placeholders::_2)
        );
        
        node_->create_service<pacsim::srv::ClockTriggerAbsolute>(
            "/pacsim/clock_trigger/absolute", 
            std::bind(&PacSimNode::handleClockTriggerAbsolute, this, std::placeholders::_1, std::placeholders::_2)
        );
        
        node_->create_service<pacsim::srv::ClockTriggerRelative>(
            "/pacsim/clock_trigger/relative", 
            std::bind(&PacSimNode::handleClockTriggerRelative, this, std::placeholders::_1, std::placeholders::_2)
        );
    }
    ...
    void updateSensors(Eigen::Vector3d& position, Eigen::Vector3d& euler_angles,
        ...
        geometry_msgs::msg::TwistWithCovarianceStamped vel_msg = createRosTwistMsg(vel, rot, "car", sim_time_);
        velocity_pub_->publish(vel_msg);
        
        ImuData imu_data_cog{
            model_->getAcceleration(),
        ...
        for (auto& gnss : gnss_sensors_) {
            if (gnss->RunTick(track_landmarks.gnssOrigin, track_landmarks.enuToTrackRotation,
                                position, euler_angles, sim_time_, model_->getVelocity(),
                                model_->getAngularVelocity(), 
                                Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(),
                                main_config_.pre_transform_track)) {
        ...
        std::vector<double> joint_msg = {
            steering_curr.FL, wheel_orientations.FL,
            steering_curr.FR, wheel_orientations.FR,
            wheel_orientations.RR, wheel_orientations.RL,
            -steering_wheel_curr
        };
        sensor_msgs::msg::JointState joint_stamped = createRosJointMsg(joint_names_, joint_msg, sim_time_);
...
int main(int argc, char** argv) {
    ...
    std::thread simulation_thread([&pacsim_node]() {
        return pacsim_node->run();
    });
