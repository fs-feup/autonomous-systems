...
from custom_interfaces.msg import EvaluatorControlData,PathPointArray,Velocities,ConeArray,Pose,ControlParameters,PerceptionParameters,PlanningParameters,SlamParameters,VelocityEstimationParameters
...
class DataInfrastructureNode(Node):
    def __init__(self):
        ...
        self.get_logger().info('Subscribing the control/... publishers.')
        #Control publishers
        self.create_subscription(EvaluatorControlData,'/control/evaluator_data', self.common_callback, 10)
        self.create_subscription(Marker,'/control/visualization/closest_point',self.common_callback,10)
        self.create_subscription(Marker,'/control/visualization/lookahead_point',self.common_callback,10)
        #Planning publishers
        self.create_subscription(PathPointArray,'/path_planning/path',self.common_callback,10)
        self.create_subscription(Float64,'/path_planning/execution_time',self.common_callback,10)
        self.create_subscription(Marker,'/path_planning/smoothed_path',self.common_callback,10)
        self.create_subscription(MarkerArray,'/path_planning/triangulations',self.common_callback,10)
        #Slam publishers
        self.create_subscription(ConeArray,'/state_estimation/map',self.common_callback, 10)
        self.create_subscription(Pose,'/state_estimation/vehicle_pose',self.common_callback, 10)
        self.create_subscription(MarkerArray,'/state_estimation/visualization_map',self.common_callback, 10)
        self.create_subscription(MarkerArray,'/state_estimation/visualization_map_perception',self.common_callback, 10)
        self.create_subscription(Marker,'/state_estimation/visualization/position',self.common_callback, 10)
        self.create_subscription(Float64MultiArray,'/state_estimation/slam_execution_time',self.common_callback, 10)
        self.create_subscription(Float64MultiArray,'/state_estimation/slam_covariance',self.common_callback, 10)
        self.create_subscription(Float64,'/state_estimation/lap_counter',self.common_callback, 10)
        #Velocity Estimation publishers
        self.create_subscription(Velocities,'/state_estimation/velocities',self.common_callback,10)
        
        #Publishers
        self.control_par_pub = self.create_publisher(ControlParameters, 'data_infrastructure/control_parameters', 10)
        self.perception_par_pub = self.create_publisher(PerceptionParameters, 'data_infrastructure/perception_parameters', 10)
        self.planning_par_pub = self.create_publisher(PlanningParameters, 'data_infrastructure/planning_parameters', 10)
        self.slam_par_pub = self.create_publisher(SlamParameters, 'data_infrastructure/slam_parameters', 10)
        self.velocity_par_pub = self.create_publisher(VelocityEstimationParameters, 'data_infrastructure/velocity_estimation_parameters', 10)
...
def perception_parameters(self):
    ...
    perception_msg.horizontal_resolution = float(parameters.get("horizontal_resolution", 0.0))
    perception_msg.vertical_resolution = float(parameters.get("vertical_resolution", 0.0))
    ...
    perception_msg.acc_fov_trim_angle = float(parameters.get("acc_fov_trim_angle", 0.0))
    ...
    perception_msg.skid_min_distance_to_cone = float(parameters.get("skid_min_distance_to_cone", 0.0))
    ...
    perception_msg.radius_resolution = float(parameters.get("radius_resolution", 0.0))

    #ACCELERATION GROUND REMOVAL
    perception_msg.acc_n_angular_grids = int(parameters.get("acc_n_angular_grids", 0))
    perception_msg.acc_radius_resolution = float(parameters.get("acc_radius_resolution", 0.0))

    #SKIDPAD GROUND REMOVAL
    perception_msg.skid_n_angular_grids = int(parameters.get("skid_n_angular_grids", 0))
    perception_msg.skid_radius_resolution = float(parameters.get("skid_radius_resolution", 0.0))

    # Clustering
    perception_msg.clustering_n_neighbours = int(parameters.get("clustering_n_neighbours", 0))
    perception_msg.clustering_epsilon = float(parameters.get("clustering_epsilon", 0.0))
    ...
    perception_msg.height_out_weight = float(parameters.get("height_out_weight", 0.0))
    perception_msg.height_in_weight = float(parameters.get("height_in_weight", 0.0))
    perception_msg.cylinder_radius_weight = float(parameters.get("cylinder_radius_weight", 0.0))
    perception_msg.cylinder_height_weight = float(parameters.get("cylinder_height_weight", 0.0))
    perception_msg.cylinder_npoints_weight = float(parameters.get("cylinder_npoints_weight", 0.0))
    perception_msg.npoints_weight = float(parameters.get("npoints_weight", 0.0))
    perception_msg.displacement_x_weight = float(parameters.get("displacement_x_weight", 0.0))
    perception_msg.displacement_y_weight = float(parameters.get("displacement_y_weight", 0.0))
    perception_msg.displacement_z_weight = float(parameters.get("displacement_z_weight", 0.0))
    perception_msg.deviation_xoy_weight = float(parameters.get("deviation_xoy_weight", 0.0))
    perception_msg.deviation_z_weight = float(parameters.get("deviation_z_weight", 0.0))
...
def planning_parameters(self):
    ...
    planning_msg.outliers_spline_order = int(parameters.get("outliers_spline_order", 0))
    planning_msg.outliers_spline_coeffs_ratio = float(parameters.get("outliers_spline_coeffs_ratio", 0.0))
    planning_msg.outliers_spline_precision = int(parameters.get("outliers_spline_precision", 0))
    planning_msg.path_calculation_dist_threshold = float(parameters.get("path_calculation_dist_threshold", 0.0))
    planning_msg.smoothing_spline_order = int(parameters.get("smoothing_spline_order", 0))
    ...
    planning_msg.braking_acceleration = float(parameters.get("braking_acceleration", 0.0))
    planning_msg.normal_acceleration = float(parameters.get("normal_acceleration", 0.0))
    planning_msg.use_velocity_planning = bool(parameters.get("use_velocity_planning", False))
...
def slam_parameters(self):
    ...
    slam_msg.data_association_model_name = parameters.get("data_association_model_name", "")
    slam_msg.data_association_limit_distance = float(parameters.get("data_association_limit_distance", 0.0))
    slam_msg.data_association_gate = float(parameters.get("data_association_gate", 0.0))
    slam_msg.new_landmark_confidence_gate = float(parameters.get("new_landmark_confidence_gate", 0.0))
    ...
    slam_msg.angular_velocity_noise = float(parameters.get("angular_velocity_noise", 0.0))
    slam_msg.pose_x_initial_noise = float(parameters.get("pose_x_initial_noise", 0.0))
    slam_msg.pose_y_initial_noise = float(parameters.get("pose_y_initial_noise", 0.0))
    slam_msg.pose_theta_initial_noise = float(parameters.get("pose_theta_initial_noise", 0.0))
    slam_msg.slam_solver_name = parameters.get("slam_solver_name", "")
    slam_msg.minimum_observation_count = int(parameters.get("minimum_observation_count", 0))
    slam_msg.minimum_frequency_of_detections = int(parameters.get("minimum_frequency_of_detections", 0))
    slam_msg.slam_min_pose_difference = float(parameters.get("slam_min_pose_difference", 0.0))
    ...
    slam_msg.slam_optimization_period = float(parameters.get("slam_optimization_period", 0.0))
    slam_msg.slam_isam2_relinearize_threshold = float(parameters.get("slam_isam2_relinearize_threshold", 0.0))
    slam_msg.slam_isam2_relinearize_skip = int(parameters.get("slam_isam2_relinearize_skip", 0))
    slam_msg.slam_isam2_factorization = parameters.get("slam_isam2_factorization", "")
...
def velocity_parameters(self):
    ...
    velocity_msg.imu_acceleration_noise = float(parameters.get("imu_acceleration_noise", 0.0))
    velocity_msg.imu_rotational_noise = float(parameters.get("imu_rotational_noise", 0.0))
