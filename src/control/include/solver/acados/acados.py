...
def load_mpc_parameters():
    ...
    wheel_speeds_scale = global_config["control"]["wheel_speeds_scale_mpc"]
...
def export_mpc_model() -> AcadosModel:
    ...
    # Smooth regularization factor (prevents division by zero)
    eps_vx = 0.5

    # Regularized longitudinal velocities (strictly positive)
    # Breaks lateral dynamics if velocity is negative
    v_fl_x_reg = sqrt(v_fl_x**2 + eps_vx**2)
    v_fr_x_reg = sqrt(v_fr_x**2 + eps_vx**2)
    v_rl_x_reg = sqrt(v_rl_x**2 + eps_vx**2)
    v_rr_x_reg = sqrt(v_rr_x**2 + eps_vx**2)

    # Slip angles at each wheel (No if_else needed)
    slip_angle_fl = sa_fl - atan2(v_fl_y, v_fl_x_reg)
    slip_angle_fr = sa_fr - atan2(v_fr_y, v_fr_x_reg)
    slip_angle_rl = sa_rl - atan2(v_rl_y, v_rl_x_reg)
    slip_angle_rr = sa_rr - atan2(v_rr_y, v_rr_x_reg)
    ...
    # Regularized wheel-frame longitudinal velocities
    vx_fl_reg = sqrt(vx_fl**2 + eps_vx**2)
    vx_fr_reg = sqrt(vx_fr**2 + eps_vx**2)
    ...
    f_expl = vertcat(
        ...
        (acceleration_y - x[7]) / 10, # simplified dynamics
        steering_dot,
