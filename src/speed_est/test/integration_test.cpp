...
class ExecTimeTest : public ::testing::Test {
 protected:
  ...
  std::shared_ptr<rclcpp::Node> se_node_test;
  ...
  std::string workload;
  void print_to_file() {
    std::ofstream file("../../performance/exec_time/speed_est.csv",
                       std::ios::app);  // append
  ...
  void SetUp() override {  // TODO(PedroRomao3) //SetUpTestSuite
    ...
    cones_publisher = receiver_publisher_mock->create_publisher<custom_interfaces::msg::ConeArray>(
        ...
        10); /**< Publishes cone array that will be received by SENode */
    ...
    se_node_test = std::make_shared<SENode>(ekf_test, perception_map_test, motion_update_test,
...
TEST_F(ExecTimeTest, PUBLISH_INTEGRATION_TEST_1X10CONE) {
  ...
  executor.add_node(se_node_test);
  start_time = std::chrono::high_resolution_clock::now();
  while (!done) {
