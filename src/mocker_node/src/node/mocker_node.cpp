...
MockerNode::MockerNode() : rclcpp::Node("mocker_node") {
  ...
  std::string se_gtruth_default_file_path = std::string(std::filesystem::current_path()) +
                                         "/gtruths/tracks_gtruths/track1/track1.csv";
  std::string planning_gtruth_file =
      declare_parameter<std::string>("planning_gtruth_file_path", planning_gtruth_default_file_path);
  std::string se_gtruth_file =
      declare_parameter<std::string>("se_gtruth_file_path", se_gtruth_default_file_path);
  gtruth_planning = planning_gtruth_fromfile(openFileAsStream(planning_gtruth_file));
  gtruth_se = se_gtruth_fromfile(openFileAsStream(se_gtruth_file));
  ...
  this->timer_ = this->create_wall_timer(
      std::chrono::milliseconds(100), std::bind(&MockerNode::publish_data, this));
