...
MockerNode::MockerNode(const std::string &track_name, const std::string &sim) : rclcpp::Node("mocker_node") {
  std::string planning_gtruth_default_file_path =
      std::string(std::filesystem::current_path()) +
      "/gtruths/tracks/" +  sim + "/" + track_name + "/" + track_name + "_gtruth.csv";
  std::string se_gtruth_default_file_path =
      std::string(std::filesystem::current_path()) + "/gtruths/tracks/" + sim + "/" + track_name + "/" + track_name + ".csv";
