...
/**
 * @brief Adapter class for the InvictaSim.
 *
 * This class subscribes to the IMU, wheel speed sensors, and steering angle topics published by the
 * InvictaSim. It then passes the data to the velocity estimator for processing.
 */
class InvictaSimAdapter : public VENode {
...
public:
  explicit InvictaSimAdapter(const VEParameters& parameters);
  /**
