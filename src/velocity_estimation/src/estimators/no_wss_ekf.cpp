...
NoWSSEKF::NoWSSEKF(const VEParameters& params) {
  ...
  this->_motor_and_steering_measurement_noise_matrix_ = Eigen::MatrixXd::Identity(2, 2);
  this->_motor_and_steering_measurement_noise_matrix_(0, 0) = params.steering_angle_noise_;
  this->_motor_and_steering_measurement_noise_matrix_(1, 1) = params.motor_rpm_noise_;
...
void NoWSSEKF::wss_callback(const common_lib::sensor_data::WheelEncoderData& wss_data) { return; }

void NoWSSEKF::motor_rpm_callback(double motor_rpm) {
  this->motor_rpm_ = motor_rpm;
  this->motor_rpm_received_ = true;
  if (this->steering_angle_received_) {
    this->correct_motor_and_steering(this->_state_, this->_covariance_, this->motor_rpm_,
...
void NoWSSEKF::steering_callback(double steering_angle) {
  this->steering_angle_ = steering_angle;
  this->steering_angle_received_ = true;
  if (this->motor_rpm_received_) {
    this->correct_motor_and_steering(this->_state_, this->_covariance_, this->motor_rpm_,
                                     this->steering_angle_);
    this->steering_angle_received_ = false;
    this->motor_rpm_received_ = false;
...
common_lib::structures::Velocities NoWSSEKF::get_velocities() {
...
void NoWSSEKF::predict(Eigen::Vector3d& state, Eigen::Matrix3d& covariance,
...
void NoWSSEKF::correct_motor_and_steering(Eigen::Vector3d& state, Eigen::Matrix3d& covariance,
  ...
  Eigen::MatrixXd kalman_gain = covariance * jacobian.transpose() *
                                (jacobian * covariance * jacobian.transpose() +
                                 this->_motor_and_steering_measurement_noise_matrix_)
                                    .inverse();

  // DEBUG PRINTS
  RCLCPP_INFO(rclcpp::get_logger("velocity_estimation"),
              "correct_motor_and_steering - Predicted observations: %f %f ",
              predicted_observations(0), predicted_observations(1));
  RCLCPP_INFO(rclcpp::get_logger("velocity_estimation"),
              "correct_motor_and_steering - Observations: %f %f", observations(0), observations(1));
  RCLCPP_INFO_STREAM(rclcpp::get_logger("velocity_estimation"),
                     "correct_motor_and_steering - Covariance: \n"
                         << covariance);
  RCLCPP_INFO_STREAM(rclcpp::get_logger("velocity_estimation"), "correct_motor_and_steering - y: \n"
                                                                    << y);
  RCLCPP_INFO_STREAM(rclcpp::get_logger("velocity_estimation"),
                     "correct_motor_and_steering - Jacobian: \n"
                         << jacobian);
  RCLCPP_INFO_STREAM(rclcpp::get_logger("velocity_estimation"),
                     "correct_motor_and_steering - Kalman gain: \n"
                         << kalman_gain);
  state += kalman_gain * y;
  RCLCPP_INFO_STREAM(rclcpp::get_logger("velocity_estimation"),
                     "correct_motor_and_steering - New State: \n"
                         << state);
  if (this->_has_made_prediction_)
    covariance = (Eigen::Matrix3d::Identity() - kalman_gain * jacobian) * covariance;
}

void NoWSSEKF::correct_imu(Eigen::Vector3d& state, Eigen::Matrix3d& covariance,
                           common_lib::sensor_data::ImuData& imu_data) {
