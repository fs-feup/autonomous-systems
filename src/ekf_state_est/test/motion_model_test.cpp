...
//   Eigen::MatrixXf g_matrix =
//       motion_model.get_motion_to_state_matrix(Eigen::VectorXf::Zero(10), prediction_data, 1.0);
//   Eigen::MatrixXf new_covariance = g_matrix * Eigen::MatrixXf::Ones(10, 10) *
//   g_matrix.transpose(); EXPECT_DOUBLE_EQ(new_state(0), 1.0);  // x EXPECT_DOUBLE_EQ(new_state(1),
//   0.0);  // y EXPECT_DOUBLE_EQ(new_state(2), 0.0);  // theta for (int i = 0; i < 10; i++) {
//     for (int j = 0; j < 10; j++) {
//       EXPECT_DOUBLE_EQ(new_covariance(i, j), 1.0);
//     }
//   }
