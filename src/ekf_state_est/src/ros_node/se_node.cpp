...
SENode::SENode() : Node("ekf_state_est") {
  ...
  if (data_assocation_model_name == "max_likelihood") {
    float gate_1 = static_cast<float>(this->declare_parameter("data_association_gate1", 5.991f));
    float gate_2 = static_cast<float>(this->declare_parameter("data_association_gate2", 30.0f));
    MaximumLikelihood::nis_gate_ = gate_1;
    MaximumLikelihood::nd_gate_ = gate_2;
  }
  float observation_noise = static_cast<float>(this->declare_parameter("observation_noise", 0.05f));

  int data_association_limit_distance =
      static_cast<int>(this->declare_parameter("data_association_limit_distance", 15));

  float wheel_speed_sensor_noise =
      static_cast<float>(this->declare_parameter("wheel_speed_sensor_noise", 0.003f));

  float IMU_noise = static_cast<float>(this->declare_parameter("IMU_noise", 0.00000064f));

  std::shared_ptr<MotionModel> motion_model_wss =
      motion_model_constructors.at("normal_velocity_model")(
          MotionModel::create_process_noise_covariance_matrix(wheel_speed_sensor_noise));  // TUNE
  RCLCPP_INFO(this->get_logger(), "MOTION MODEL WSS CREATED\n");
  std::shared_ptr<MotionModel> motion_model_imu =
      motion_model_constructors.at("imu_velocity_model")(
          MotionModel::create_process_noise_covariance_matrix(IMU_noise));  // TUNE
  RCLCPP_INFO(this->get_logger(), "IMU MODEL WSS CREATED\n");
  std::shared_ptr<ObservationModel> observation_model = std::make_shared<ObservationModel>(
      ObservationModel::create_observation_noise_covariance_matrix(observation_noise));  /// TUNE
  // rclcpp info data data_assocation_model_name
  RCLCPP_INFO(this->get_logger(), "Data Association Model Name: %s",
              data_assocation_model_name.c_str());
  ...
  RCLCPP_INFO(this->get_logger(), "DA MODEL WSS CREATED\n");
  ...
  RCLCPP_INFO(this->get_logger(), "Adapter\n");
}
