...
void ExtendedKalmanFilter::correction_step() {
  ...
  for (auto cone : this->_map_from_perception->map) {
    ObservationData observation_data = ObservationData(this->_vehicle_state->pose.position.x, this->_vehicle_state->pose.position.y, cone.second);
...
unsigned int ExtendedKalmanFilter::discovery(const ObservationData& observation_data) {
  ...
  Eigen::Vector2f landmark_absolute = this->_observation_model.inverse_observation_model(this->X, observation_data);
