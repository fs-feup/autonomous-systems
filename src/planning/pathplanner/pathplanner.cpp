...
void PathPlanner::middlePath(){
    ...
    for (int i = 0; i < track->getLeftConesSize(); i++){
        ...
        float dist = track->getRightConeAt(count  % rightConesSize)->getDistanceTo(leftCone); // previous cone distance
        ...
        while(true){
            ...
            if (new_dist > dist) break; // if new cone is not closer to the left one the previous, break(near cone found),
