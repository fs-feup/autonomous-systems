...
void PathCalculation::createMidPoints(
    std::vector<Cone>& cone_array, std::vector<std::unique_ptr<MidPoint>>& midPoints,
    std::unordered_map<MidPoint*, std::vector<Point>>& triangle_points) {
  DT dt;
  ...
  for (Finite_edges_iterator it = dt.finite_edges_begin(); it != dt.finite_edges_end(); ++it) {
    ...
    auto midPoint =
        std::make_unique<MidPoint>(MidPoint{Point((p1.x() + p2.x()) / 2, (p1.y() + p2.y()) / 2),
                                            {},
                                            &cone_array[id1],
                                            &cone_array[id2]});
    ...
    auto collect_edges = [&](auto face, int skip_edge_index) {
      for (int i = 0; i < 3; ++i) {
        ...
        (void)neighbor_edges.emplace_back(face->vertex((i + 1) % 3)->point(),
...
void PathCalculation::connectMidPoints(
  ...
  for (const auto& q : midPoints) {
    segment_to_midpoint[ordered_segment(Point(q->cone1->position.x, q->cone1->position.y),
                                        Point(q->cone2->position.x, q->cone2->position.y))] =
        q.get();
  }
...
void PathCalculation::selectInitialPath(
    std::vector<Point>& path, const std::vector<std::unique_ptr<MidPoint>>& midPoints,
  ...
  if (path_to_car.size() > 2) {
    RCLCPP_INFO(rclcpp::get_logger("planning"), "Selecting initial path from %zu points.",
                path_to_car.size());

    Point snapped_first = path_to_car[0];

    if (MidPoint* mp_first =
            find_nearest_point(snapped_first, point_to_midpoint, config_.tolerance_);
        mp_first) {
    ...
    for (std::size_t i = 1; i < path_to_car.size(); ++i) {
      ...
      MidPoint* current_mp =
          find_nearest_point(current_point, point_to_midpoint, config_.tolerance_);
      if (current_mp) {
