...
std::pair<std::vector<Cone>, std::vector<Cone>> Outliers::approximate_cones_with_spline(
  ...
  if (config_.use_cone_colouring_ && config_.use_outlier_removal_) {
  ...
  } else if (config_.use_cone_colouring_) {  // if (!config_.use_outlier_removal_)
    for (auto& cone : cones.first) cone.color = Color::BLUE;
    for (auto& cone : cones.second) cone.color = Color::YELLOW;
    return cones;
  } else {  // if (!config_.use_cone_colouring_)
    return cones;
  }
}