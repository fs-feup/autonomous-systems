 ...
 *
 * This class inherits from rclcpp::Node and provides functionalities for handling different
 * missions, subscribing to vehicle localization and track map topics, and publishing planned path
 * points.
 */
class Planning : public rclcpp::Node {
   ...
   * @details creates track with the received cones(position), then calculates path by calling
   * method from the local path planner and finnaly publishes the path by calling the publish method
   */
   ...
   * @details This function publishes the provided path points as a custom created PointArray
   * message.
   */
   ...
   * @details Depending on the selected mission, this function publishes predicted path points
   * (published the path for the missions we previously know the track for).
   */
 ...
 public:
   ...
   *
   * Initializes an instance of the Planning class (a ROS node) with essential communication
   * components.
   *
   * This constructor sets up subscriptions to vehicle localization and mapping topics, and creates
   * publishers for local and (global X) path planning results. It also establishes a timer for
   * periodic tasks related to (local) publishing info to topics. Additionally, it initializes an
   * Adapter instance for communication with external systems.
   */
  Planning();
  /**
   ...
   * @details This method configures the Planning node for a specific mission type, possibly
   * affecting its behavior if used.
   */
