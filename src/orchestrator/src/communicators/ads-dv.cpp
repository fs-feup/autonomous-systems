...
AdsDvCommunicator::AdsDvCommunicator() {
  if (fs_ai_api_init("can0", 1, 1)) {
    printf("fs_ai_api_init() failed\r\n");
    return 1;
  }
}

custom_interfaces::msg::vehicle_info AdsDvCommunicator::read_from_car() {
  fs_ai_api_vcu2ai_get_data(&vcu2ai_data);

  auto msg = std::make_unique<custom_interfaces::msg::vehicle_info>();
...
void AdsDvCommunicator::send_to_car(const custom_interfaces::msg::Ai2Vcu::SharedPtr msg) {
  // Send handshake bit
