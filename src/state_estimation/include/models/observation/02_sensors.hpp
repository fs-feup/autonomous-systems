...
public:
  ...
  void update_imu_data(const common_lib::sensor_data::ImuData& imu_data) {
    last_imu_data_ = imu_data;
  }
  void update_wss_data(const common_lib::sensor_data::WheelEncoderData& wss_data) {
    last_wss_data_ = wss_data;
  }
  void update_motor_rpm(double motor_rpm) { last_motor_rpm_ = motor_rpm; }
  void update_steering_angle(double steering_angle) { last_steering_angle_ = steering_angle; }
