#pragma once

#include "common_lib/sensor_data/imu.hpp"
#include "common_lib/sensor_data/wheel_encoders.hpp"
#include "common_lib/structures/velocities.hpp"
/**
 * @brief Interface for state estimators
 *
 * This class defines the interface for state estimators. State estimators are used to
 * estimate the vehicle's state based on sensor measurements.
 */
class StateEstimator {
public:
  /**
   * @brief callback for control commands that the SENode should call when new control commands are
   * received
   */
  virtual void control_callback(const common_lib::structures::ControlCommand& control_command) = 0;

  /**
   * @brief callback for IMU data that the SENode should call when new IMU data is received
   */
  virtual void imu_callback(const common_lib::sensor_data::ImuData& imu_data) = 0;

  /**
   * @brief callback for wheel speed sensor data that the SENode should call when new wheel speed
   * sensor data is received
   */
  virtual void wss_callback(const common_lib::sensor_data::WheelEncoderData& wss_data) = 0;

  /**
   * @brief callback for motor RPM data that the SENode should call when new motor RPM data is
   * received
   */
  virtual void motor_rpm_callback(double motor_rpm) = 0;

  /**
   * @brief callback for steering angle data that the SENode should call when new steering angle
   * data is received
   */
  virtual void steering_callback(double steering_angle) = 0;

  /**
   * @brief callback for timer that the SENode should call at a fixed frequency to trigger state
   * estimation updates
   */
  virtual void timer_callback() = 0;

  /**
   * @brief function to publish the estimated state that the SENode should call after each update
   */
  virtual void publish_state() = 0;
};