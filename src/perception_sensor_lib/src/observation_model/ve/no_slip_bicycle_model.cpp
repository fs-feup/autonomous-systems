...
Eigen::VectorXd NoSlipBicycleModel::expected_observations(
  ...
  double rear_wheel_velocity = sqrt(pow(vx, 2) + pow(omega * lr, 2));
  ...
  double front_wheel_velocity = sqrt(pow(vx, 2) + pow(omega * lf, 2));
...
Eigen::MatrixXd NoSlipBicycleModel::expected_observations_jacobian(
  ...
  jacobian(0, 2) = sign * 60 * lf * lf * omega /
  ...
  jacobian(2, 2) = sign * 60 * lr * lr * omega /
  ...
  jacobian(5, 0) = sign * this->car_parameters_->gear_ratio * 60 * vx /
                   (M_PI * this->car_parameters_->wheel_diameter * rear_wheel_velocity);
  jacobian(5, 2) = sign * this->car_parameters_->gear_ratio * 60 * lr * lr * omega /
                   (M_PI * this->car_parameters_->wheel_diameter * rear_wheel_velocity);
