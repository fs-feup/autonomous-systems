...
Eigen::VectorXi MaximumLikelihoodNLL::associate(
    const Eigen::VectorXd& landmarks, const Eigen::VectorXd& observations, const Eigen::MatrixXd& covariance, const Eigen::VectorXd& observation_confidences) const {
  ...
  for (int i = 0; i < num_observations; ++i) {
    ...
    for (int j = 0; j < num_landmarks; j++) {
      ...
      Eigen::Matrix2d innovation_covariance = covariance.block(2*j, 2*j, 2, 2) + this->observation_noise_covariance_matrix_;
