...
RANSAC2::RANSAC2(const double epsilon, const int n_tries) 
    : epsilon(epsilon), n_tries(n_tries) {}
...
void RANSAC2::ground_removal(const pcl::PointCloud<pcl::PointXYZI>::Ptr point_cloud,
    ...
    // Calculate the best plane using RANSAC
    Plane best_plane = calculate_plane(point_cloud, plane);
    
    // Update the output plane parameter
    plane = best_plane;
...
Plane RANSAC2::calculate_plane(const pcl::PointCloud<pcl::PointXYZI>::Ptr point_cloud, 
    ...
    for (int i = 0; i < n_tries; ++i) {
        ...
        // Ensure we get 3 different points
        do {
            for (int j = 0; j < 3; ++j) {
                indices[j] = dis(gen);
        ...
        if (target_plane.normal.x != 0 || target_plane.normal.y != 0 || target_plane.normal.z != 0) {
            double angle_diff = calculate_angle_difference(candidate_plane, target_plane);
            double angle_diff_degrees = angle_diff * (180.0 / M_PI);
            
            // Skip if angle difference is too large (assuming 20 degrees like in Python)
            if (angle_diff_degrees > 20.0) {
                continue;
            }
        }
        ...
        if (!best_plane_found || inliers > best_plane_inliers) {
            ...
            std::cout << "New best plane found with " << inliers 
                      << " inliers and max deviation " << max_deviation << std::endl;
...
Plane RANSAC2::fit_plane_to_points(const std::vector<pcl::PointXYZI>& points) const {
    ...
    Plane plane;
    plane.normal = normal;
    plane.point = Eigen::Vector3d(p1.x, p1.y, p1.z);
...
double RANSAC2::calculate_angle_difference(const Plane& plane1, const Plane& plane2) const {
    double dot_product = plane1.normal.dot(plane2.normal);
