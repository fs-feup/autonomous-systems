...
bool ConeEvaluator::evaluateCluster(Cluster &cluster, Plane &ground_plane) const {
  ...
  condifence += evaluator_weights_.at("height_out_weight") * cur_validator_results.at(0) +
  ...
  condifence += evaluator_weights_.at("cylinder_radius_weight") * cur_validator_results.at(0) +
                evaluator_weights_.at("cylinder_height_weight") * cur_validator_results.at(1) +
                evaluator_weights_.at("cylinder_points_weight") * cur_validator_results.at(2);
  ...
  condifence += evaluator_weights_.at("npoints_weight") * cur_validator_results.at(0);
  ...
  condifence += evaluator_weights_.at("displacement_x_weight") * cur_validator_results.at(0) +
  ...
  condifence += evaluator_weights_.at("deviation_xoy_weight") * cur_validator_results.at(0) +
                evaluator_weights_.at("deviation_z_weight") * cur_validator_results.at(1);

  cluster.set_confidence(condifence);
  return condifence >= min_confidence_;
}