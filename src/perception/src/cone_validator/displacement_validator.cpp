...
std::vector<double> DisplacementValidator::coneValidator(Cluster* cone_cluster,
  ...
  float minX = *reinterpret_cast<const float*>(&cloud_data[PointX(indices[0])]);
  float maxX = minX;
  float minY = *reinterpret_cast<const float*>(&cloud_data[PointY(indices[0])]);
  float maxY = minY;
  float minZ = *reinterpret_cast<const float*>(&cloud_data[PointZ(indices[0])]);
  float maxZ = minZ;
