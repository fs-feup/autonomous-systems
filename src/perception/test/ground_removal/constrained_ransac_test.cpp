...
protected:
  void SetUp() override {
    ...
    cloud.points.push_back(pcl::PointXYZI{0.005, 0.005, 0.005, 2.0});
    cloud.points.push_back(pcl::PointXYZI{10.0, 10.0, 10.0, 3.0});

    cloud_3_points.points.clear();
    cloud_3_points.points.push_back(pcl::PointXYZI{0.0, 0.0, 0.0, 0.5});
    cloud_3_points.points.push_back(pcl::PointXYZI{1.0, 0.0, 0.0, 1.0});
    cloud_3_points.points.push_back(pcl::PointXYZI{0.0, 1.0, 0.0, 1.5});
...
TEST_F(ConstrainedRANSACTest, TooFewPointsThrows) {
  ...
  EXPECT_THROW(ransac.ground_removal(empty_ptr, output_ptr, base_plane, split_params),
