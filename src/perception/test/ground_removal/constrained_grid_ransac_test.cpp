...
public:
  void SetUp() override {
    ...
    cloud.points.push_back(PointXYZIR{0.0060, 0.0060, 0.0060, 2.0});
    cloud.points.push_back(PointXYZIR{10, 10, 10, 2.5});

    empty_cloud.points.clear();

    cloud_3_points.points.clear();
    cloud_3_points.points.push_back(PointXYZIR{1.0, 0.0, 0.0, 0.5});
    cloud_3_points.points.push_back(PointXYZIR{0.0, 1.0, 0.0, 1.0});
    cloud_3_points.points.push_back(PointXYZIR{0.0, 0.0, 1.0, 1.5});

    cloud_two_grids.points.clear();
    cloud_two_grids.points.push_back(PointXYZIR{55.0, 55.0, 100.0, 0.5});
    cloud_two_grids.points.push_back(PointXYZIR{60.0, 60.0, 100.0, 1.0});
    cloud_two_grids.points.push_back(PointXYZIR{55.0, 55.0, 100.0, 1.5});
    cloud_two_grids.points.push_back(PointXYZIR{-55.0, -55.0, 0.0, 0.5});
    cloud_two_grids.points.push_back(PointXYZIR{-50.0, -50.0, 0.0, 1.0});
    cloud_two_grids.points.push_back(PointXYZIR{-60.0, -60.0, 0.0, 1.5});
...
TEST_F(ConstrainedGridRANSACTest, TestTwoGrids) {
  ...
  const pcl::PointCloud<PointXYZIR>::Ptr cloud_ptr(&cloud_two_grids,
                                                   NonOwningDeleter<pcl::PointCloud<PointXYZIR>>());
  const pcl::PointCloud<PointXYZIR>::Ptr output_ptr(
      &ground_removed_cloud, NonOwningDeleter<pcl::PointCloud<PointXYZIR>>());
