...
Eigen::Vector4d PacejkaCombinedSlip::tire_forces(const TireInput& tire_input) {
  ...
  double Dx =
      this->car_parameters_->tire_parameters->tire_D_longitudinal * tire_input.vertical_load;
  ...
  double Fx0 = Dx * std::sin(Cx * std::atan(Bx * tire_input.slip_ratio -
                                            Ex * (Bx * tire_input.slip_ratio -
                                                  std::atan(Bx * tire_input.slip_ratio))));
  ...
  double Fy0 = -Dy * std::sin(Cy * std::atan(By * effective_slip_angle -
                                             Ey * (By * effective_slip_angle -
  ...
  // This model ignores the self aligning moment and the rolling resistance moment
  return Eigen::Vector4d(Fx, Fy, 0.0, 0.0);
}
