...
common_lib::structures::Wheels RigidBodyLoadTransferModel::compute_loads(
  ...
  double longitudinal_transfer = calculate_elastic_longitudinal_transfer(longitudinal_acceleration);
...
double RigidBodyLoadTransferModel::calculate_elastic_longitudinal_transfer(
    double longitudinal_acceleration) const {
  // This model has no separate anti-squat/anti-dive geometry term, so the entire
  // longitudinal transfer is treated as spring-reacted.
  double mass = this->car_parameters_->total_mass;
  double g = 9.8065;  // m/s^2 TODO: make this a constant in the car parameters (maybe?)
  double cog_height = this->car_parameters_->cg_height;
  double wheelbase = this->car_parameters_->wheelbase;
  return (mass * longitudinal_acceleration * cog_height) / wheelbase;
}