...
common_lib::structures::Wheels VDLoadTransferModel::compute_loads(
  ...
  right tires same for aero. Front static load returns the value of both tires Aero load front
  returns the value of both tires Longitudinal transfer returns the total value of load transfer
  that is then distributed between front and rear Lateral transfer returns the total value of load
  transfer that is then distributed between each axis
  */
  double front_left_load = (front_static_load / 2) + (aero_load_front / 2) -
                           (longitudinal_transfer / 2) - lateral_transfer_front;  // Front Left
  double front_right_load = (front_static_load / 2) + (aero_load_front / 2) -
                            (longitudinal_transfer / 2) + lateral_transfer_front;  // Front Right
  double rear_left_load = (rear_static_load / 2) + (aero_load_rear / 2) +
                          (longitudinal_transfer / 2) - lateral_transfer_rear;  // Rear Left
  double rear_right_load = (rear_static_load / 2) + (aero_load_rear / 2) +
                           (longitudinal_transfer / 2) + lateral_transfer_rear;  // Rear Right
  return common_lib::structures::Wheels(front_left_load, front_right_load, rear_left_load,
                                        rear_right_load);
}
...
double VDLoadTransferModel::calculate_elastic_longitudinal_transfer(
    double longitudinal_acceleration) const {
  return ((car_parameters_->sprung_mass) * longitudinal_acceleration *
...
double VDLoadTransferModel::calculate_longitudinal_transfer(
  ...
  double unsprung_load_transfer = ((car_parameters_->unsprung_mass) * longitudinal_acceleration *
                                  ...
                                  car_parameters_->wheelbase;
  double elastic_load_transfer = calculate_elastic_longitudinal_transfer(longitudinal_acceleration);
